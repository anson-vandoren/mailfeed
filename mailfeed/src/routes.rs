            ...
            web::scope("/api")
                .service(
                    web::scope("/users")
                        ...
                        // TODO: creating a user requires an admin-authenticated request
                        ...
                        .service(
                            web::scope("/{user_id}/subscriptions")
                                .route("", web::get().to(handlers::subscription::get_all_subscriptions))
                                .route("", web::post().to(handlers::subscription::create_subscription))
                                .route("/{id}", web::get().to(handlers::subscription::get_subscription))
                                .route("/{id}", web::put().to(handlers::subscription::update_subscription))
                                .route("/{id}", web::delete().to(handlers::subscription::delete_subscription))
                        )
                )
                .service(
                    web::scope("/auth")
                        ...
                        .route("/password-reset", web::post().to(handlers::auth::password_reset))
                )
                .service(
                    web::scope("/feeds")
